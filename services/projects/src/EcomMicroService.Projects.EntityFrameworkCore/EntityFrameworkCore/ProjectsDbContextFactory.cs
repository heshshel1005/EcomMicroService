...
public class ProjectsDbContextFactory : IDesignTimeDbContextFactory<ProjectsDbContext>
{
    ...
    {
        return BuildConfiguration().GetConnectionString(ProjectsDbProperties.ConnectionStringName) ?? "Host=127.0.0.1;Port=5432;Database=ef_design;Username=postgres;Password=postgres";
    }
        ...
        var builder = new ConfigurationBuilder()
            .SetBasePath(
                Path.Combine(
                    ...
                    $"host{Path.DirectorySeparatorChar}EcomMicroService.Projects.HttpApi.Host"
                )
            )
