...
public class ProjectsHttpApiHostModule : AbpModule
{
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        ...
        context.ConfigureMicroservice(EcomMicroServiceNames.ProjectsApi);
            ...
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                options.FileSets.ReplaceEmbeddedByPhysical<ProjectsDomainSharedModule>(
                    Path.Combine(
                        ...
                        string.Format(
                            "..{0}..{0}src{0}EcomMicroService.Projects.Domain.Shared",
                ...
                options.FileSets.ReplaceEmbeddedByPhysical<ProjectsDomainModule>(
                    Path.Combine(
                        hostingEnvironment.ContentRootPath,
                        string.Format(
                            "..{0}..{0}src{0}EcomMicroService.Projects.Domain",
                            Path.DirectorySeparatorChar
                        )
                    )
                );
                options.FileSets.ReplaceEmbeddedByPhysical<ProjectsApplicationContractsModule>(
                    Path.Combine(
                        hostingEnvironment.ContentRootPath,
                        string.Format(
                            "..{0}..{0}src{0}EcomMicroService.Projects.Application.Contracts",
                            Path.DirectorySeparatorChar
                        )
                    )
                );
                options.FileSets.ReplaceEmbeddedByPhysical<ProjectsApplicationModule>(
                    Path.Combine(
                        hostingEnvironment.ContentRootPath,
                        string.Format(
                            "..{0}..{0}src{0}EcomMicroService.Projects.Application",
                            Path.DirectorySeparatorChar
                        )
                    )
                );
