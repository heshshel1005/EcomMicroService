...
public class MarketingHttpApiHostModule : AbpModule
{
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        ...
        context.ConfigureMicroservice(EcomMicroServiceNames.MarketingApi);
            ...
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                options.FileSets.ReplaceEmbeddedByPhysical<MarketingDomainSharedModule>(
                    Path.Combine(
                        ...
                        string.Format(
                            "..{0}..{0}src{0}EcomMicroService.Marketing.Domain.Shared",
                ...
                options.FileSets.ReplaceEmbeddedByPhysical<MarketingDomainModule>(
                    Path.Combine(
                        hostingEnvironment.ContentRootPath,
                        string.Format(
                            "..{0}..{0}src{0}EcomMicroService.Marketing.Domain",
                            Path.DirectorySeparatorChar
                        )
                    )
                );
                options.FileSets.ReplaceEmbeddedByPhysical<MarketingApplicationContractsModule>(
                    Path.Combine(
                        hostingEnvironment.ContentRootPath,
                        string.Format(
                            "..{0}..{0}src{0}EcomMicroService.Marketing.Application.Contracts",
                            Path.DirectorySeparatorChar
                        )
                    )
                );
                options.FileSets.ReplaceEmbeddedByPhysical<MarketingApplicationModule>(
                    Path.Combine(
                        hostingEnvironment.ContentRootPath,
                        string.Format(
                            "..{0}..{0}src{0}EcomMicroService.Marketing.Application",
                            Path.DirectorySeparatorChar
                        )
                    )
                );
