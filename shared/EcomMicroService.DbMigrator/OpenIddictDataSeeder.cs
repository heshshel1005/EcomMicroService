...
using EcomMicroService.DbMigrator.Model;
using Volo.Abp;
...
) : ITransientDependency
{
    ...
    {
        var type = string.IsNullOrWhiteSpace(secret)
        ...
        var existing = await _applicationManager.FindByClientIdAsync(name);
        var application = new OpenIddictApplicationDescriptor
        {
            ClientId = name,
            ClientType = type,
            ApplicationType = OpenIddictConstants.ApplicationTypes.Web,
            ClientSecret = existing == null ? secret : null,
            ...
            foreach (var grantType in grantTypes)
            {
                ...
                if (
                    grantType
                    is OpenIddictConstants.GrantTypes.AuthorizationCode
                        or OpenIddictConstants.GrantTypes.Implicit
                )
                {
                    application.Permissions.Add(
                ...
                if (
                    grantType
                    is OpenIddictConstants.GrantTypes.AuthorizationCode
                        or OpenIddictConstants.GrantTypes.ClientCredentials
                        or OpenIddictConstants.GrantTypes.Password
                        or OpenIddictConstants.GrantTypes.RefreshToken
                        or OpenIddictConstants.GrantTypes.DeviceCode
                )
                {
        ...
        {
            var update = new OpenIddictApplicationDescriptor();
            await _applicationManager.PopulateAsync(update, existing);
            update.ClientType = type;
            update.ApplicationType = OpenIddictConstants.ApplicationTypes.Web;
            update.ConsentType = consentType;
            update.DisplayName = displayName;
            update.Permissions.Clear();
            foreach (var permission in application.Permissions)
            {
                update.Permissions.Add(permission);
            }
            update.RedirectUris.Clear();
            foreach (var uri in application.RedirectUris)
            {
                update.RedirectUris.Add(uri);
            }
            update.PostLogoutRedirectUris.Clear();
            foreach (var uri in application.PostLogoutRedirectUris)
            {
                update.PostLogoutRedirectUris.Add(uri);
            }

            await _applicationManager.UpdateAsync(existing, update);
        }
    }
}
